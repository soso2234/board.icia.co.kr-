...
<c:if test="${!empty list}">   
   <c:forEach var="qna" items="${list}" varStatus="status">   
   	<c:choose>
   	<c:when test="${qna.qnaHiBbsSeq eq 9999}">
	    <tr>
		   <td class="text-center">공지사항</td>
		   <td><a href="javascript:void(0)" onclick="fn_view(${qna.qnaHiBbsSeq})">
			   <c:if test="${qna.qnaHiBbsIndent > 0}">
			   	<img src="/resources/images/icon_reply.gif" style="margin-left: ${qna.qnaHiBbsIndent};"/></c:if>
      ...
      <c:if test="${qna.qnaHiBbsIndent > 0}">
         <img src="/resources/images/icon_reply.gif" style="margin-left: ${qna.qnaHiBbsIndent};"/>
      </c:if>
               <c:out value="${qna.qnaHiBbsTitle}" />
            </a>
         </td>
